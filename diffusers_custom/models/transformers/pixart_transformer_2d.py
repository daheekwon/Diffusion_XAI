        ...
        for i,block in enumerate(self.transformer_blocks):
            if self.training and self.gradient_checkpointing:
                ...
                if ablate_channels is not None:
                    if (i != 27) and (ablate_channels[i] is not None):
                        ch = ablate_channels[i]
                        hidden_states[:,:,ch] = 0
                if return_latents:
                    features.append(hidden_states[1,:,:].detach().cpu().numpy())
                # print(i,hidden_states.shape)
